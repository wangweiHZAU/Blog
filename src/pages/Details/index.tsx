...
import { ArticleInfo, IArticle } from '../../types'
...
export default function Details(props: RouteComponentProps<{ id: string }>) {
  ...
  const getArticle = async () => {
    let res = await api.getArticle(props.match.params.id)
    if (res.data.stat === 'ok') {
      setArticle(res.data.data.info)
  ...
  return (
    <Form onFinish={publicArticle}>
      <Form.Item
        ...
        <Input style={{ width: '60%' }} maxLength={50} value={article?.title} />
      </Form.Item>
      ...
      <Form.Item label="封面" name="image">
        <Cover
          ...
          defaultCover={article ? article.banner[0] : ''}
          key="cover"
        />
      </Form.Item>
      <Form.Item>
